...
def test_report_lookup_by_normalized_name_uses_latest_profile_when_season_is_blank():
    report = build_player_scouting_report(
        recommendation_data=make_recommendation_pipeline_data(),
        performance_df=make_short_term_rows("points"),
        long_term_df=make_long_term_rows(),
    ...
    assert report["player"]["season"] == "2025-26"


def test_report_keeps_profile_when_expected_forecast_is_unavailable():
    report = build_player_scouting_report(
        recommendation_data=make_recommendation_pipeline_data(),
        performance_df=make_short_term_rows("points"),
        long_term_df=make_long_term_rows(),
    ...
    assert any("Short-term forecast unavailable" in warning for warning in report["warnings"])
